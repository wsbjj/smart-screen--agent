...
describe('electron preload bridge wiring', () => {
  ...
  it('exposes the full desktop API from a sandbox-compatible preload script', async () => {
    ...
    expect(preloadSource).toContain("'jobs:list'")
    expect(preloadSource).toContain("'jobs:save'")
    expect(preloadSource).toContain("'jobs:delete'")
    expect(preloadSource).toContain('list: () => ipcRenderer.invoke(channels.listJobs)')
    expect(preloadSource).toContain('save: (input) => ipcRenderer.invoke(channels.saveJob, input)')
    expect(preloadSource).toContain('delete: (id) => ipcRenderer.invoke(channels.deleteJob, id)')
